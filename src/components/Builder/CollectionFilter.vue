<template>
  <div>
    <div class="d-flex mb-2">
      ...
      <b-btn v-b-modal.modalPacks variant="outline-secondary" size="sm" class="ml-1">Packs</b-btn>
    </div>
    ...
    <b-form-checkbox-group
      ...
      class="btn-group btn-clans d-flex my-2"
      role="group"
      aria-label="Clan Filter"
    >
      <b-form-checkbox
        @click.native="onClick('clan', $event)"
        v-for="clan in options.clan"
        :key="clan.id"
        class="btn-sm col"
        :class="'clan-'+clan.id"
        :value="clan.id"
    ...
    <b-form-checkbox-group
      ...
      buttons
      button-variant="outline-secondary"
      class="btn-group d-flex my-2"
      role="group"
      aria-label="Deck Filter"
    >
      <b-form-checkbox
        @click.native="onClick('side', $event)"
        v-for="side in options.side"
        :key="side.id"
        class="btn-sm col"
        :value="side.id"
        :title="$t('side.' + side.id)"
      >{{ $t('side.' + side.id).substr(0, 1) }}</b-form-checkbox>
    </b-form-checkbox-group>
    ...
    >
      <b-form-checkbox
        @click.native="onClick('type', $event)"
...
export default {
  ...
  methods: {
    ...
    getSideOptions() {
      return ["dynasty", "conflict", "province", "role"].map(sideId => ({
        id: sideId,
...
.btn-clans .btn.clan-crab:hover,
.btn-clans .btn.active.clan-crab {
  background: rgb(0, 28, 148);
  color: white;
}

.btn-clans .btn.clan-crane:hover,
.btn-clans .btn.active.clan-crane {
  background: rgb(0, 137, 222);
  color: white;
}

.btn-clans .btn.clan-dragon:hover,
.btn-clans .btn.active.clan-dragon {
  background: rgb(0, 164, 114);
  color: white;
}

.btn-clans .btn.clan-lion:hover,
.btn-clans .btn.active.clan-lion {
  background: rgb(203, 157, 0);
  color: white;
}

.btn-clans .btn.clan-phoenix:hover,
.btn-clans .btn.active.clan-phoenix {
  background: rgb(193, 100, 0);
  color: white;
}

.btn-clans .btn.clan-scorpion:hover,
.btn-clans .btn.active.clan-scorpion {
  background: rgb(166, 22, 0);
  color: white;
}

.btn-clans .btn.clan-unicorn:hover,
.btn-clans .btn.active.clan-unicorn {
  background: rgb(120, 0, 152);
  color: white;
}

.btn-clans .btn.clan-neutral:hover,
.btn-clans .btn.active.clan-neutral {
